...
public class GameScene extends PixelScene {
	...
	public void create() {
		...
		else if (Dungeon.depth >= 21 && Dungeon.depth < 25)
		{
			...
			else if (Dungeon.extrastage_Sea) Music.INSTANCE.play(Assets.Music.GAME6_IBERIA1, true);
		...
		if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
			...
			if (Dungeon.hero.hasTalent(Talent.ROGUES_FORESIGHT)
				...
				Random.pushGenerator(Dungeon.seedCurDepth());
					if (reqSecrets <= 0 && Random.Int(5) <= 1 + Dungeon.hero.pointsInTalent(Talent.ROGUES_FORESIGHT)){
